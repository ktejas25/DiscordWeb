...
-- Server members policies (fixed to avoid recursion)
CREATE POLICY "Users can view members of servers they belong to"
  ...
  USING (user_id = auth.uid() OR server_id IN (
    SELECT server_id FROM api.server_members WHERE user_id = auth.uid()
  ));
...
CREATE POLICY "Server owners can add members"
  ...
  WITH CHECK (
    server_id IN (
      SELECT id FROM api.servers WHERE owner_id = auth.uid()
    )
  );
